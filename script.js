...
function initColorSettings() {
  ...
  syntaxElements.forEach(element => {
    ...
    // 初始化时更新预览
    previewElement.style.backgroundColor = currentColor;
    ...
    colorInput.addEventListener('input', () => {
      ...
      // 更新对应输入框的值
      const hexInput = document.querySelector(`#${element.id}Color + input[type="text"]`);
      if (hexInput) {
        hexInput.value = newColor;
      }
      
      saveColorSetting(element.id, newColor);
      applyColorSettings();
    });
  });
  
  // 为所有现有的颜色输入框添加变更监听
  document.querySelectorAll('.color-setting input[type="color"]').forEach(colorInput => {
    const colorId = colorInput.id.replace('Color', '');
    const previewElement = document.getElementById(`${colorId}Preview`);
