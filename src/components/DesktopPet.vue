<template>
  <div 
    ...
    :style="{
      ...
      zIndex: 9999,
      // 【新增】哭泣时禁止所有交互，非哭泣时恢复
      pointerEvents: isCrying ? 'none' : 'auto'
    }"
  ...
  >
    <!-- 【修改】根据哭泣状态切换桌宠图片 -->
    <img 
      :src="isCrying ? './audio/cryingpet.png' : './audio/pet.png'" 
      alt="Desktop Pet"
      class="pet-image"
      draggable="false"
      @dragstart.prevent
    ...
    <!-- 【新增】哭泣时隐藏拖拽提示 -->
    <div v-if="showHint && !isCrying" class="pet-hint">
      上半区抖动，下半区拖拽，右下角拉伸
    </div>
...
const {
  ...
  isBottomHalf,
  isCrying, // 【新增】解构哭泣状态
  triggerCrying, // 【新增】解构触发哭泣方法
  stopCrying // 【新增】解构停止哭泣方法
} = useDesktopPet()

// 提示显示
const showHint = ref(true)

// 音频相关 - 【修改】解构useAudio中的专属方法，弃用通用playSound
const { playPetSound, playCrySound } = useAudio()
// 原有宠物抖动音频播放方法
const playPetAudio = () => {
  try {
    playPetSound() // 调用useAudio中专门的宠物音效方法，避免重复创建实例
  } catch (e) {
...
// 【新增】监听哭泣状态变化 - 触发时播放哭泣音频，结束后复原
watch(isCrying, (newVal) => {
  if (newVal && petContainer.value) {
    // 哭泣状态：播放哭泣音频
    const cryAudio = playCrySound()
    // 监听音频播放结束事件，结束后停止哭泣、复原所有状态
    if (cryAudio) {
      cryAudio.addEventListener('ended', () => {
        stopCrying(petContainer.value)
...
/* 【修改】哭泣摇晃动画 - 移除scale(3)，仅保留旋转，持续7秒 */
:deep(.desktop-pet) {
  &[style*="pointer-events: none"] {
    animation: cryShake 7s linear infinite;
  }
}
